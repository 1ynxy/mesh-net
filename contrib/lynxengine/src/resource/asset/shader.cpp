...
bool Shader::load_pass(std::string data, GLenum type) {
    ...
    if (pass == 0) {
        debug.error("failed to create shader " + define);
...
void Shader::set(const std::string& name, float val) {
    ...
    glUniform1fv(get_uniform(name), 1, &val);
}

void Shader::set(const std::string& name, glm::vec2 val) {
    if (!ready()) return;

    glUniform2fv(get_uniform(name), 1, glm::value_ptr(val));
}

void Shader::set(const std::string& name, glm::vec3 val) {
    if (!ready()) return;

    glUniform3fv(get_uniform(name), 1, glm::value_ptr(val));
}

void Shader::set(const std::string& name, glm::vec4 val) {
    if (!ready()) return;

    glUniform4fv(get_uniform(name), 1, glm::value_ptr(val));
}
